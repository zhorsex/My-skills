...
def validate_skill(skill_path):
    ...
    # 读取并验证前置元数据 (Frontmatter)
    content = skill_md.read_text(encoding='utf-8')
    if not content.startswith('---'):
        return False, "未发现 YAML 前置元数据"
    ...
    name = name.strip()
    if name:
        # 检查命名规范 (hyphen-case: 小写字母加连字符)
        if not re.match(r'^[a-z0-9-]+$', name):
            return False, f"名称 '{name}' 应为横线格式 (仅限小写字母、数字和连字符)"
        if name.startswith('-') or name.endswith('-') or '--' in name:
            return False, f"名称 '{name}' 不能以连字符开头/结尾，或包含连续连字符"
        # 检查名称长度 (根据规范，最长 64 个字符)
        if len(name) > 64:
            return False, f"名称太长 ({len(name)} 个字符)。最大长度为 64 个字符。"
    ...
    if description:
        ...
        # 检查描述长度 (根据规范，最长 1024 个字符)
        if len(description) > 1024:
            return False, f"描述太长 ({len(description)} 个字符)。最大长度为 1024 个字符。"
